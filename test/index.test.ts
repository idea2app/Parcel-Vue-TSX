...
test('build output differs from default TSX pipeline and emits source map', () => {
  execSync('pnpm exec parcel build index.vapor.tsx --dist-dir dist --no-cache --no-optimize --log-level error', {
    cwd: vaporDir,
    stdio: 'inherit'
  });

  const jsAsset = readdirSync(join(vaporDir, 'dist')).find(name => name.endsWith('.js'));
  const sourceMapAsset = readdirSync(join(vaporDir, 'dist')).find(name => name.endsWith('.map'));
  ...
  execSync('pnpm exec parcel build index.tsx --dist-dir dist --no-cache --no-optimize --log-level error', {
    cwd: baselineDir,
    stdio: 'inherit'
  });
